...
public class BoardAdjTargetTest {
	// We make the Board static because we can load it one time and 
		...
		public void testAdjacenciesRooms()
		{
			...
			// First, the kitchen that only has a single door but a secret room
			...
			// now test the Pre-labour room (note not marked since multiple test here)
			testList = board.getAdjList(3, 4);
			assertEquals(3, testList.size());
			assertTrue(testList.contains(board.getCell(7, 4)));
			
			// one more room, the manager's office
		...
		public void testTargetsOccupied() {
			...
			board.getCell(14, 9).setOccupied(true);
			board.calcTargets(board.getCell(12, 9), 2);
			board.getCell(14, 9).setOccupied(false);
			Set<BoardCell> targets = board.getTargets();
			assertEquals(5, targets.size());
			assertTrue(targets.contains(board.getCell(11, 8)));
			assertTrue(targets.contains(board.getCell(11, 10)));
			assertTrue(targets.contains(board.getCell(10, 9)));	
			assertFalse( targets.contains( board.getCell(14, 9))) ;
		
			// we want to make sure we can get into a room, even if flagged as occupied
			board.getCell(3, 4).setOccupied(true);
			board.getCell(7, 3).setOccupied(true);
			board.calcTargets(board.getCell(7, 4), 1);
			board.getCell(3, 4).setOccupied(false);
			...
			board.calcTargets(board.getCell(3, 4), 2);
			board.getCell(7, 4).setOccupied(false);
			targets= board.getTargets();
			assertEquals(4, targets.size());
